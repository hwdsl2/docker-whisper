...
- 最低内存：默认 `base` 模型约需 500 MB 可用内存（请参阅[模型列表](#切换模型)）
...
**注意：** 在 `env` 文件中，值可用单引号括起，例如 `VAR='value'`。`=` 两侧不要有空格。如更改 `WHISPER_PORT`，请相应更新 `docker run` 命令中的 `-p` 参数。
...
**注意：** 如需面向公网部署，强烈建议使用[反向代理](#使用反向代理)启用 HTTPS。此时请将 `docker-compose.yml` 中的 `"9000:9000/tcp"` 改为 `"127.0.0.1:9000:9000/tcp"`，以防止未加密端口被直接访问。
...
| `response_format` | 字符串 | — | 输出格式，默认为 `json`。请参阅[响应格式](#响应格式)。 |
...
```
/var/lib/whisper/
├── hub/                  # 缓存的 Whisper 模型文件（从 HuggingFace 下载）
...
**注意：** 预构建镜像中包含的软件组件（如 faster-whisper 及其依赖项）均受各自版权持有者所选许可证约束。使用预构建镜像时，用户有责任确保其使用方式符合镜像内所有软件的相关许可证要求。
