...
WHISPER_IDLE_UNLOAD_SECONDS=$(nospaces "$WHISPER_IDLE_UNLOAD_SECONDS")
WHISPER_IDLE_UNLOAD_SECONDS=$(noquotes "$WHISPER_IDLE_UNLOAD_SECONDS")
...
[ -z "$WHISPER_IDLE_UNLOAD_SECONDS" ] && WHISPER_IDLE_UNLOAD_SECONDS=0
...
if ! printf '%s' "$WHISPER_IDLE_UNLOAD_SECONDS" | grep -Eq '^(0|[1-9][0-9]*)$'; then
  exiterr "WHISPER_IDLE_UNLOAD_SECONDS must be 0 (disabled) or a positive integer."
fi
...
USAGE_STATE_DIR="${DATA_DIR}/.whisper-usage"
USAGE_BASE_URL=${WHISPER_USAGE_BASE_URL:-https://github.com/hwdsl2/ai-stack-extras/releases/download/v1.0.0}
data_mounted=false
data_existing=false
...
report_usage_counts() {
  ...
  [ "${WHISPER_DISABLE_USAGE_COUNTS:-0}" != "1" ] || return 0
  ...
  state_file="$USAGE_STATE_DIR/whisper.version"
  ...
  if [ -n "$action" ]; then
    if fetch_usage_asset "cu-v1-whisper-$action-$accel-$arch"; then
...
export WHISPER_IDLE_UNLOAD_SECONDS
